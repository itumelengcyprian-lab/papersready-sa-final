// PapersReady SA V14 - BLURRY SKIP AFTER 2 TRIES + KHAKI + ANTI-CHANCER
...
final txIdController=TextEditingController(); final providerIdController=TextEditingController(); final voucherDateController=TextEditingController();
...
int blurryAttempts=0; bool showSkipOption=false; // NEW - BLURRY COUNTER
...
Future<void> checkSassaStatus() async {
  ...
  try{
    ...
    if(!mounted) return;
    showDialog(context: context, builder: (c)=>AlertDialog(title: const Text('SASSA Portal - Data OK', style:TextStyle(fontSize:12, color:Colors.green, fontWeight:FontWeight.bold)), content: Text('ID ${idController.text} ready.\nOpening srd.sassa.gov.za/sc19/status', style: const TextStyle(fontSize:10)), actions: [TextButton(onPressed: ()=>Navigator.pop(c), child: const Text('Close')), ElevatedButton(onPressed: () async { Navigator.pop(c); final url=Uri.parse('https://srd.sassa.gov.za/sc19/status'); if(await canLaunchUrl(url)) await launchUrl(url, mode: LaunchMode.externalApplication); }, child: const Text('Open SASSA Site'))]));
  } catch(e){
    if(!mounted) return;
    showDialog(context: context, builder: (c)=>AlertDialog(title: const Text('Opening SASSA'), content: Text('We will open official site. Error: $e', style: const TextStyle(fontSize:10)), actions: [ElevatedButton(onPressed: () async { Navigator.pop(c); final url=Uri.parse('https://srd.sassa.gov.za/sc19/status'); if(await canLaunchUrl(url)) await launchUrl(url, mode: LaunchMode.externalApplication); }, child: const Text('Open SASSA Site'))]));
...
Future<void> verifyAndGoToStep2() async {
 ...
 if(nameController.text.trim().isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Enter full name under Affidavit section')));return;}
 ...
 if(postalController.text.trim().isEmpty || postalController.text.length!=4){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Enter 4-digit postal code')));return;}
 ...
 if(!isDevTest){
   if(tx.length!=10 || int.tryParse(tx)==null){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Transaction ID must be exactly 10 digits')));return;}
   if(usedTxIds.contains(tx)){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('This Transaction ID already used! Old slip not allowed.')));return;}
   ...
   if(voucherDateController.text.trim().isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Enter voucher date from slip')));return;}
   if(slipPath==null){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Upload CLEAR slip photo')));return;}
   File slipFile = File(slipPath!);
   int size = await slipFile.length();
   if(size < 20000){
     setState(()=>blurryAttempts++);
     if(blurryAttempts>=2){
       setState(()=>showSkipOption=true);
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Slip still blurry after 2 tries! You can now Skip & contact support'), backgroundColor:Colors.orange));
     } else {
       ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Slip not clear! Attempt $blurryAttempts/2 - Photo too blurry/small. Try again with clearer photo'), backgroundColor:Colors.red));
     }
     return;
   }
 }
...
Future<void> sendToWhatsApp() async {
 ...
 try{ await Share.shareXFiles([XFile(f.path)], text:'Your ${selectedLetter} - PapersReady SA'); }catch(e){}
...
void contactSupportForSlip(){
  ...
  final msg=Uri.encodeComponent('Hello PapersReady SA Support (083 925 8423),\n\nI bought R20 MoMo voucher but my slip photo is blurry/not uploading after 2 tries.\n\nMy details:\nName: ${nameController.text}\nID: ${idController.text}\nTxID: ${txIdController.text}\nProvider ID: ${providerIdController.text}\nTown: $selectedTown\n\nPlease help me verify manually. I have slip photo.\n\nThank you.');
...
Widget buildStep1(){
 return SingleChildScrollView(padding:const EdgeInsets.all(12), child:Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
  ...
  Container(padding:const EdgeInsets.all(10), decoration:BoxDecoration(color:Colors.white, border:Border.all(color:const Color(0xFF8D7B4A), width:1.5), borderRadius:BorderRadius.circular(8)), child: Column(crossAxisAlignment:CrossAxisAlignment.start, children: [
    const Text('Affidavit Details - Fill Here (Top left of letter):', style:TextStyle(fontWeight:FontWeight.bold, fontSize:11, color:Color(0xFF8D7B4A))),
    const SizedBox(height:8),
  ...
  const SizedBox(height:8),
  TextField(controller:voucherDateController, decoration:const InputDecoration(labelText:'Voucher Date & Time * (From slip)', border:OutlineInputBorder(), isDense:true, filled:true, fillColor:Colors.white)),
  const SizedBox(height:8),
  SizedBox(width:double.infinity, child:ElevatedButton.icon(onPressed:() async { final XFile? f=await ImagePicker().pickImage(source:ImageSource.gallery); if(f!=null) setState(()=>{slipPath=f.path, blurryAttempts=0, showSkipOption=false}); }, icon:const Icon(Icons.upload), label:Text(slipPath==null?'Upload CLEAR Slip Photo *':'Slip Selected ✓ - ${slipPath!.split('/').last.substring(0,15)}...'), style:ElevatedButton.styleFrom(backgroundColor: slipPath==null? Colors.orange : Colors.green, foregroundColor:Colors.white))),
  ...
  if(showSkipOption) Container(margin: const EdgeInsets.only(top:8), padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.red[50], border: Border.all(color: Colors.red), borderRadius: BorderRadius.circular(8)), child: Column(children: [
    ...
    const Text('If you BOUGHT voucher but uploading has problem, you can skip and contact support directly:', style:TextStyle(fontSize:9)),
  ...
  const SizedBox(height:8),
  const Text('If slip not clear / no Provider ID / old voucher - will be skipped. For buyer with upload problem after 2 tries → Skip → Contact support same number.', style:TextStyle(fontSize:8, color:Colors.red, fontWeight:FontWeight.bold)),
  const SizedBox(height:12),
...
Widget buildStep2(){
 return SingleChildScrollView(padding:const EdgeInsets.all(12), child:Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
  ...
  SizedBox(width:double.infinity, height:50, child:ElevatedButton.icon(icon:verifying?const SizedBox(width:18,height:18,child:CircularProgressIndicator(color:Colors.white,strokeWidth:2)):const Icon(Icons.print, size:20), label:Text(verifying?'Generating...':'Generate Final PDF & Send to WhatsApp', style:const TextStyle(fontSize:11, fontWeight:FontWeight.bold)), style:ElevatedButton.styleFrom(backgroundColor:Colors.green, foregroundColor:Colors.white), onPressed:verifying?null:sendToWhatsApp)),
  const SizedBox(height:10),
...
class PrivacyScreen extends StatelessWidget{const PrivacyScreen({super.key}); @override Widget build(BuildContext context){ return Scaffold(appBar: AppBar(title: const Text('Privacy Policy'), backgroundColor:const Color(0xFF8D7B4A), foregroundColor:Colors.white), body: const SingleChildScrollView(padding: EdgeInsets.all(16), child: Text('Privacy - PapersReady SA\nContact: itumelengcyprian@gmail.com', style:TextStyle(fontSize:11))));}}
